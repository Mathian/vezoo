...
function _renderWaitingQr() {
  ...
  if (phone) renderQrCode('op-waiting-qr', phone, 160);
  if (qrPhn) qrPhn.textContent = phone || '—';
}

async function acceptVenueInvite() {
  if (!checkRateLimit('opAcceptInvite', 1, 10000)) { showToast('Слишком часто', 'warning'); return; }
  if (!_invite) return;
...
async function opOpenAssignCourier(orderId) {
  ...
  const allCouriers=await dbQuery('couriers','status','==','active');
  const available=allCouriers.filter(c=>permUids.includes(c.uid));
  if (listEl) {
    if (!available.length) { listEl.innerHTML=`<div class="empty" style="padding:24px"><div class="empty-icon">🚴</div><div class="empty-text">Нет постоянных курьеров.</div></div>`; return; }
...
async function opFindHandoffCourier() {
  if (!checkRateLimit('opFindHandoff', 1, 3000)) { showToast('Слишком часто', 'warning'); return; }
  ...
  foundEl.innerHTML=`<div class="handoff-courier-found"><div class="handoff-courier-avatar">🚴</div><div><div style="font-weight:700;font-size:15px">${courier.name||'Курьер'}</div><div style="font-size:13px;color:var(--text-dim)">${courier.phone||phone}</div></div></div>`;
...
async function loadOpSettings() {
  ...
  if (phone) renderQrCode('op-qr-img', phone, 180);
